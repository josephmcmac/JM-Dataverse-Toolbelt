...
namespace JosephM.XrmModule.ToolingConnector
{
    ...
    public partial class ExampleLoginForm : Window
    {
        ...
        {
            //when this is loadedin vsix it seems to use a an explicit path for the assembly which doesnt resolve
            //adding this explicit loading via a type in the assembly seems to fix the problem
            if(args.Name == "Microsoft.Xrm.Tooling.Ui.Styles, Culture=neutral")
                return typeof(Microsoft.Xrm.Tooling.Ui.Styles.WindowResourceDictionary).Assembly;
            return null;
        }
